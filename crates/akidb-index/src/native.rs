...
impl VectorStore {
    fn new(dimension: usize, distance: DistanceMetric) -> Self {
        Self {
...
impl IndexProvider for NativeIndexProvider {
    ...
    async fn build(&self, request: BuildRequest) -> Result<IndexHandle> {
        ...
        let store = VectorStore::new(dimension, request.distance);
    ...
    fn deserialize(&self, bytes: &[u8]) -> Result<IndexHandle> {
        ...
        let collection = "restored".to_string(); // TODO: Store collection in serialized data
